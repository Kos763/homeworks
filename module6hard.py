...
class Triangle(Figure):
    ...
    def __init__(self, color, *sides):
        super().__init__(color, *sides)
        self.sides = sides
        f = (self.sides[0] + self.sides[1] + self.sides[2]) / 2
        self.__height = 2 * (sqrt(f * (f - self.sides[0]) * (f - self.sides[1]) * (f - self.sides[2]))) / self.sides[0]

    def get_square(self):
        g = (self.__height * self.sides[0]) / 2
        return g
